# REFACTORED: This monolithic views.py has been split into modular components.
# All view functions are now in predictor/views/ directory:
#   - system_views.py: Authentication & home endpoints
#   - animal_views.py: Animal prediction & clustering
#   - bird_views.py: Bird prediction & clustering
#   - insect_views.py: Insect prediction & clustering
#   - plant_views.py: Plant prediction & features
#   - analytics_views.py: Advanced clustering & analytics
#
# All URL patterns are now consolidated in predictor/urls.py
#
# Original file: predictor/views_old.py (backup of 2327-line monolithic version)
# This stub file is kept to prevent Python package conflicts with the views/ directory.
...
def _load_occurrence_classifier_if_needed(category: str):
    ...
    try:
        ...
        occurrence_models[category] = joblib.load(str(model_files['occurrence_classifier']))
        try:
            loaded_features = joblib.load(str(model_files['occurrence_features']))
...
# Load animals model
try:
    animals_model = joblib.load(str(wildlife_config.ANIMALS_MODEL_PKL))
    animals_scaler = joblib.load(str(wildlife_config.ANIMALS_SCALER_PKL))
    animals_features = joblib.load(str(wildlife_config.ANIMALS_FEATURE_NAMES_PKL))
...
try:
    try:
        birds_features = joblib.load(str(wildlife_config.BIRDS_FEATURE_NAMES_PKL))
    except Exception:
        birds_features = BASE_BIRDS_FEATURES.copy()

    birds_scaler = joblib.load(str(wildlife_config.BIRDS_SCALER_PKL))
    birds_model = joblib.load(str(wildlife_config.BIRDS_MODEL_PKL))
    try:
        birds_metadata = joblib.load(str(wildlife_config.BIRDS_METADATA_PKL))
    except Exception:
        birds_metadata = {}
...
try:
    try:
        insects_features = joblib.load(str(wildlife_config.INSECTS_FEATURE_NAMES_PKL))
    except Exception:
        insects_features = BASE_INSECTS_FEATURES.copy()

    insects_scaler = joblib.load(str(wildlife_config.INSECTS_SCALER_PKL))
    insects_model = joblib.load(str(wildlife_config.INSECTS_MODEL_PKL))
    try:
        insects_metadata = joblib.load(str(wildlife_config.INSECTS_METADATA_PKL))
    except Exception:
        insects_metadata = {}
...
try:
    try:
        plants_features = joblib.load(str(wildlife_config.PLANTS_FEATURE_NAMES_PKL))
    except Exception:
        plants_features = BASE_PLANTS_FEATURES.copy()

    plants_scaler = joblib.load(str(wildlife_config.PLANTS_SCALER_PKL))
    plants_model  = joblib.load(str(wildlife_config.PLANTS_MODEL_PKL))
    try:
        plants_metadata = joblib.load(str(wildlife_config.PLANTS_METADATA_PKL))
    except Exception:
        plants_metadata = {}
    try:
        plants_kmeans        = joblib.load(str(wildlife_config.PLANTS_KMEANS_PKL))
        plants_kmeans_scaler = joblib.load(str(wildlife_config.PLANTS_KMEANS_SCALER_PKL))
    except Exception:
        plants_kmeans = None
...
def _build_gallery_rows_from_csv(csv_name: str, default_subtitle: str) -> list[dict]:
    ...
    try:
        # Map CSV names to wildlife_config paths
        csv_mapping = {
            'koyna_birds_regression_density.csv': str(wildlife_config.PROCESSED_BIRDS_REGRESSION_CSV),
            'koyna_insects_regression_density.csv': str(wildlife_config.PROCESSED_INSECTS_REGRESSION_CSV),
            'koyna_animals_regression_density.csv': str(wildlife_config.PROCESSED_ANIMALS_REGRESSION_CSV),
            'koyna_plants_regression_density.csv': str(wildlife_config.PROCESSED_PLANTS_REGRESSION_CSV),
        }
        csv_path = csv_mapping.get(csv_name, str(Path(wildlife_config.BACKEND_DIR) / csv_name))
...
def _get_insects_thresholds():
    ...
    try:
        df = pd.read_csv(str(wildlife_config.PROCESSED_INSECTS_REGRESSION_CSV))
...
def get_plants_features(request):
    ...
    try:
        ...
        regression_path = Path(wildlife_config.PROCESSED_PLANTS_REGRESSION_CSV)
        if regression_path.exists():
            ...
            raw_df = pd.read_csv(str(wildlife_config.RAW_PLANTS_CSV))
...
def get_plants_model_info(request):
    ...
    km_meta = {}
    try:
        km_meta = joblib.load(str(wildlife_config.PLANTS_KMEANS_META_PKL))
    except Exception:
        pass
    return JsonResponse({
        'regression': {
...
def get_animals_features(request):
    ...
    try:
        df = pd.read_csv(str(wildlife_config.PROCESSED_ANIMALS_REGRESSION_CSV))
