...
def train_model_for_category(category_name, csv_file, target_col, prefix):
    ...
    os.makedirs('ml_logic', exist_ok=True)
    joblib.dump(model, f'ml_logic/{prefix}_model.pkl')
    joblib.dump(scaler, f'ml_logic/{prefix}_scaler.pkl')
    joblib.dump(UNIVERSAL_FEATURES, f'ml_logic/{prefix}_feature_names.pkl')
    ...
    joblib.dump(metadata, f'ml_logic/{prefix}_metadata.pkl')
    
    # Save importance
    fi_df = pd.DataFrame({'Feature': UNIVERSAL_FEATURES, 'Importance': model.feature_importances_})\
        .sort_values('Importance', ascending=False)
    fi_df.to_csv(f'ml_logic/{prefix}_feature_importance.csv', index=False)
    print(f"✓ Artifacts saved successfully in ml_logic/")
